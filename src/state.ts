...
const DEFAULT_STATE: PuaState = Object.freeze({ enabled: false, configured: false, flavor: 'alibaba' });
export const RESULT_PREFIX = 'PUA · ';

export function transition(state: PuaState, action: Action): PuaState {
  switch (action.kind) {
    case 'on': case 'activate': case 'review': case 'again': case 'done-check': case 'evidence':
      return { ...state, enabled: true, configured: true };
    case 'off':
      return { ...state, enabled: false, configured: true };
    case 'flavor':
      return { ...state, flavor: action.flavor };
    default:
      return state;
  }
}
