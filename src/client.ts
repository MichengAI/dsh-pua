...
interface ClientContext {
  slots: { inject(name: string, register: () => () => void): () => void;
    register(options: { name: string; id?: string; key?: string; order?: number; label?: string }, render: (props: { sessionId?: string }) => React.ReactNode): () => void };
...
const modeNames: Record<string, string> = { pua: '普通 · 持续推进与证据交付', p7: 'P7 · 方案驱动', p9: 'P9 · 技术负责人', p10: 'P10 · 战略规划', pro: 'Pro · 自进化', yes: 'Yes · 鼓励', mama: 'Mama · 关怀', shot: 'Shot · 激励', 'pua-en': '英文协议', 'pua-ja': '日文协议' };
...
export function ConfigurationPanel({ remote, sessionId }: { remote: PuaRemoteApi; sessionId?: string | undefined }): React.ReactElement {
  ...
  function field(key: Key) {
    ...
    return h('div', { className: 'pua-field', key, 'data-inline': typeof value === 'boolean' || !!options }, h('div', { className: 'pua-field-head' }, h('label', { htmlFor: id }, labels[key]),
      !global && overridden && h('span', { className: 'pua-override' }, h('small', null, '已自定义'),
        h(Button, { disabled: busy, 'aria-label': `恢复${labels[key]}默认值`, onClick: () => void save({ [key]: null }) }, '恢复默认'))), input,
      descriptions[key] && h('small', null, descriptions[key]));
  }
  async function loop(start: boolean) {
    if (!sessionId || saving.current) return;
    saving.current = true; setBusy(true); setError('');
    try { const result = await unwrap(start ? remote.startLoop(sessionId, task, configSchema.parse(loopValues ?? draft)) : remote.cancelLoop(sessionId)); await refresh(); setStatus(result.text); }
    catch (reason) { setError(message(reason)); }
    finally { saving.current = false; setBusy(false); }
  }
  return h('section', { className: 'pua-panel', 'aria-label': global ? 'PUA 全局配置' : 'PUA 会话配置' },
    ...
    snapshot && h(React.Fragment, null, ['enabled', 'flavor', 'mode', 'subagents'].map(key => field(key as Key)),
      ...
      h('div', { className: 'pua-actions' }, global
        ? h(React.Fragment, null, h(Button, { variant: 'primary', disabled: busy || !dirty, onClick: () => void save() }, '保存全局配置'), h(Button, { disabled: busy, onClick: () => { ++epoch.current; setDraft({ ...CONFIG_DEFAULTS }); setDirty(true); } }, '恢复默认值'))
        : h(Button, { disabled: busy || !Object.keys(snapshot.overrides).length, onClick: () => void save(Object.fromEntries(CONFIG_KEYS.map(key => [key, null]))) }, '全部恢复默认')),
    ...
    h('p', { className: 'pua-status', role: 'status', 'aria-live': 'polite' }, dirty ? '有未保存的修改' : status));
}
