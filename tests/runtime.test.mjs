...
test('反馈只依据可见旁白，不唤醒模型；离线模式关闭提醒', async t => {
  ...
  await say('完成任务');
  const reminders = () => agent.session.snapshotEvents().filter(event => event.type === 'user/message' && JSON.stringify(event.data).includes('PUA 本地反馈（自愿）'));
...
test('连续排队Loop只允许最新命令启动循环，旧任务不会恢复旧配置', { timeout: 5000 }, async t => {
  ...
  const starts = agent.session.snapshotEvents().filter(event => event.type === 'user/message' && JSON.stringify(event.data).includes('显式启动 Loop；验证配置以用户命令为准。'));
