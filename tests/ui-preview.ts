...
const remote = {
  ...
  startLoop: () => response(() => ({ text: '测试夹具：Loop 已提交，未执行命令或调用模型。' })),
  cancelLoop: () => response(() => ({ text: '测试夹具：Loop 已取消。' })),
};
const seats = new Map<string, (props: { sessionId?: string }) => React.ReactNode>();
...
function App() {
  ...
  return h('main', null, h('header', null, h('strong', null, 'PUA 配置 · 离线组件验收'), h('p', null, '使用实际插件组件；此页面不连接本机 DSH，不修改真实配置。')),
    h('nav', null, ['settings', '会话 A', '会话 B'].map(id => h('button', { key: id, onClick: () => setPage(id), 'aria-pressed': page === id }, id === 'settings' ? '设置 → 插件 → PUA 配置' : id)), h('button', { onClick: () => { fail = true; } }, '模拟下次保存失败'), h('button', { onClick: () => { document.documentElement.dataset.theme = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark'; } }, '切换明暗主题')),
    page === 'settings' ? h('article', { key: page }, h('ul', { style: { padding: 0, listStyle: 'none' } }, seats.get('settings.plugin.item')?.({}))) : h('article', { key: page }, h('h1', null, page), h('p', null, '配置仅覆盖当前会话；打开 PUA 查看继承与自定义状态。'), h('textarea', { placeholder: '聊天输入区（仅布局示意）', rows: 4 }), h('div', { className: 'composer' }, h('span', null, '权限 · 默认'), seats.get('conversation.input.left')?.({ sessionId: page }), h('span', { style: { order: 1 } }, '专家'))));
}
